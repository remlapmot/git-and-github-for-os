...
- Despite not being recommend - it is better for you to add Anaconda/Python to your `PATH` in the installer options, i.e., check the first box on this screen (there is a similar option in the Python installer)  
    ```{r}
...
- Open Windows Terminal
  ...
  - you can see the location of the Python executable in `cmd` with `where python`/`where python3`  
    ```{r}
    knitr::include_graphics("./img/windows-cmd-path-where-python3.png")
    ``` 
